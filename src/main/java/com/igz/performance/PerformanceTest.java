...
public class PerformanceTest {
	...
	private static int PRODUCER_WORKERS = 1;
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (options.getQueueType() != QueueType.NONE) {
				...
				PRODUCER_WORKERS = options.getNumWorkers();
			}
	...
	private static void printTime(String operation, int total, StopWatch stopwatch) {
		String consumersDesc = options.getQueueType() == QueueType.NONE ? "" : String.format(" / %d consumers", options.getNumWorkers());

		System.out.println(String.format("%s %d (%d producers%s) in %s", operation, total, PRODUCER_WORKERS, consumersDesc, stopwatch));
	}
	...
	private static ArrayList<Callable<List<String>>> prepareProducers(OperationType operation, List<String> ids) {
		...
		int requestPerProducer = options.getNumEvents() / PRODUCER_WORKERS;

		for (int i = 0; i < PRODUCER_WORKERS; i++) {
