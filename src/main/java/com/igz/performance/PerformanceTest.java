...
public class PerformanceTest {
	...
	private static DatabaseDAO getDatabaseDAO() {
		return DatabaseFactory.createDatabase(options.getDatabaseType());
	}
	...
	private static Consumer getConsumer(int i) {
		Consumer consumer = getQueue().createConsumer("C" + i, getDatabaseDAO());
	...
	private static void testSelects(ArrayList<Callable<List<String>>> producers, DatabaseDAO dao, List<String> ids)
		...
		System.out.println(String.format("Retrieved %d (%d producers / %d consumers) in %s", ids.size(), options.getNumWorkers(), CONSUMER_COUNT,
				stopwatch));
	...
	private static List<String> testInserts(ArrayList<Callable<List<String>>> producers, DatabaseDAO dao) throws InterruptedException,
		...
		System.out.println(String.format("Inserted %d (%d producers / %d consumers) in %s", completedInserts, options.getNumWorkers(),
	...
	private static ArrayList<Callable<List<String>>> prepareProducers(OperationType operation, List<String> ids) {
		...
		int requestPerProducer = options.getNumEvents() / options.getNumWorkers();
