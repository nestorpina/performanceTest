...
import com.izg.test.tasks.RedisReadSetTask;

/**
 * 
 * This tests inserts a parses a json object, afterwars it "flattens" it, converting it 
 * to a one-dimension hashmap that we store as a set in a redis server using the jedis driver
 * After inserting the specified number of sets, using the number
 * of threads specified, it then retrieves each set by id 
 * 
 * <ul>NOTES :</ul> 
 * <li>Start redis server with :</li>
 * <pre>
 * ./src/redis-server
 * </pre>
 * <li>to issue basic commands :</li>
 * <pre>
 * ./src/redis-cli
 ...
 *
 */
public class RedisHmSetThreadedTest {
...
//					jedis.set(id, json);
        ...
        for (int i = 0; i < threadCount; i++) {
        	...
        	readTasks.add(new RedisReadSetTask(subList, flattenHashMap.keySet()));
